...
with tempfile.TemporaryDirectory() as td:
    ...
    ESCAPES = [
        ...
        ("a helper-validated name used as a DIRECTORY, then joined again", '[E-UNKNOWN-DIR]',
         "\ndef probe():\n    (output_path('sub') / 'e.txt').write_text('x')\n"),
        ("traversal hidden inside an f-string filename", '[E-DYNAMIC-NAME]',
        ...
        ("a computed file name not routed through the validated helper", '[E-DYNAMIC-NAME]',
         "\nlevel = 3\ndef probe():\n    (OUTPUT_DIR / f'{level}').write_text('x')\n"),
        ("`match` pattern capture rebinding a name used in a file name", '[E-DYNAMIC-NAME]',
         "\nlevel = 3\nmatch '../../escaped':\n    case level:\n        pass\ndef probe():\n    (OUTPUT_DIR / f'{level}').write_text('x')\n"),
        ("a shadowed `int` builtin behind a computed file name", '[E-DYNAMIC-NAME]',
         "\nint = lambda v: '../../escaped'\nlevel = int('3')\ndef probe():\n    (OUTPUT_DIR / f'{level}').write_text('x')\n"),
        ("OUTPUT_DIR itself shadowed by a function parameter", '[E-UNBOUND-NAME]',
         "\ndef probe(OUTPUT_DIR):\n    (OUTPUT_DIR / 'escaped.txt').write_text('x')\n"),
        ("calling the helper without importing it", '[E-HELPER]',
         "\ndef probe():\n    output_path('a.json', 'b').write_text('x')\n"),
    ]
