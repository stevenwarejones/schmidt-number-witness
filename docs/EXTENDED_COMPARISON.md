...
**This transcription is the one literature-dependent input to an otherwise exact verifier.** Everything downstream of it is machine-checked, and the verifier enumerates the relabeling orbit itself rather than trusting the certificate's list — but if these three definitions misread Eq. (1), the comparison closes the wrong family and no verifier fails.

Two independent readings now agree on it. This pass read the full-text HTML; the reviewing agent for PR #3 checked the primary PDF (Eq. (1), page 2) and confirmed the same marginal and correlator terms after shifting the paper's 1-based setting labels to the 0-based labels used here, with the branch written there as $(-1)^{\alpha_2}$, $\alpha_2\in\{0,1\}$, matching $b\in\{-1,+1\}$ above. That is two agent readings of one equation, not human validation and not an audit of the paper's other results. See `docs/PRIOR_ART.md`.
...
- `tests/test_gigena_mutations.py`: ten corruptions — dropped target, duplicated target, negative weight, unnormalized weights, false projected coordinate, displaced target, a phi direction off the unit circle, a non-deterministic "local" outcome, a local strategy of the wrong length, and a rescaled relabeling sign — each of which must reject with its own diagnostic code and with no other case's code. Four anti-degradation controls sit alongside them: the unmodified certificate must pass in the same staged layout, a verifier that refuses everything must satisfy no case, a verifier that always prints one code must not stand in for a different case, and the rescaled-sign certificate must be *accepted* by a copy of the verifier with that one guard removed.

  The last case came out of review of `661b814` and is worth recording, because the premise it enforces had been assumed rather than checked. `rename` is linear in the relabeling signs, so a sign of 3 rescales a correlator to 3, beyond anything a binary-outcome behaviour can reach. Splitting one positive-weight component into two halves carrying signs 3 and −1 leaves the average exactly unchanged, so the weight, projected-point, target and coverage checks all still passed and the malformed certificate was accepted. The shipped data were never affected — the reviewer independently validated all 3,502 relabelings actually present, and all are genuine — but `check_relabel` now rejects anything that is not two permutations of (0,1,2), two sign triples in {−1,+1} and a Boolean party exchange, with `[E-GK-BAD-RELABEL]`.
...
Different relabelings can use different simulators. This is **not** one SN2 model reproducing the full behavior Q. Such a model would contradict F(Q)>7. What matching does and does not leave open is worth stating precisely, because "nonlinear" on its own is the wrong dividing line: for a fixed relabeling and branch, *any* function of those same three statistics is reproduced too, nonlinear ones included, since the three numbers agree exactly. What remains unaddressed is a test that reads statistics beyond the matched triple — combining several relabelings or projections at once, or using other coordinates of the behaviour — together with filtering, multi-copy protocols and conditional tradeoffs. Those are open because different relabelings may be matched by different simulators, not because nonlinearity defeats exact matching.
