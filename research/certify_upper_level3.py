...
import sympy as sp
nr=[normpair(a,b) for a,b in words];nk=sorted(set(k for r in nr for k in r))
_,pivots=sp.Matrix([[r.get(k,0) for r in nr] for k in nk]).rref()
words=[words[i] for i in pivots];n=len(words)
Z=np.load(p/'inputs'/'clifford_scs_level3.npz')['psd_dual'];assert Z.shape==(n,n)
D=10**10
M=[[F(round(float((Z[i,j]+Z[j,i])/2)*D),D)+(F(1,10**8) if i==j else 0) for j in range(n)] for i in range(n)]
...
assert upper<F(28001,4000)
cert={'bound':str(upper),'simple_bound':'28001/4000','constant':str(beta),'residual_budget':str(radius),'gram_matrix':[[str(x) for x in r] for r in M],'words':words,'coefficients':aa+bb+sum(E,[]),'construction':'rationalized positive Gram plus exact residual operator-norm budget; valid for traceless qubit projective observables'}
(OUTPUT_DIR/'qubit_upper_level3_certificate.json').write_text(json.dumps(cert,indent=2))
