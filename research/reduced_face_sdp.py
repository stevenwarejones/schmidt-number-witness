...
old=np.load(Path(__file__).resolve().parent/'inputs'/'face_solution.npz')['X']
ee,ev=np.linalg.eigh(old);q=np.rint(3*ev[:,0]/max(abs(ev[:,0]))).astype(int)
V=sp.Matrix.hstack(*sp.Matrix([list(q)]).nullspace());P=P@np.array(V,dtype=float)
print('extra guessed rational kernel',q.tolist(),flush=True)
...
np.savez_compressed((OUTPUT_DIR/'reduced_face_map.npz'),A=Amap,target=target,P=P)
...
if X.value is not None:np.savez_compressed((OUTPUT_DIR/'reduced_face_solution.npz'),X=X.value,P=P,A=Amap,target=target,selected=sel)
