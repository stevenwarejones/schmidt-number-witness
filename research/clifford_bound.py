...
(OUTPUT_DIR/f'clifford_level{level}.json').write_text(json.dumps(out,indent=2))
if y.value is not None:np.savez((OUTPUT_DIR/f'clifford_level{level}.npz'),moments=y.value,psd_dual=constraints[0].dual_value,normalization_dual=constraints[1].dual_value,equality_dual=constraints[2].dual_value,R=R.toarray())
