...
problem.solve(solver='CLARABEL',tol_gap_abs=1e-7,tol_feas=1e-7,tol_gap_rel=1e-7,max_iter=200)
...
output_path(f'clifford_level{level}.json').write_text(json.dumps(out,indent=2))
if y.value is not None:np.savez(output_path(f'clifford_level{level}.npz'),moments=y.value,psd_dual=constraints[0].dual_value,normalization_dual=constraints[1].dual_value,equality_dual=constraints[2].dual_value,R=R.toarray())
